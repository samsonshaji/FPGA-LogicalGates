...
if {[is_project_open]} {
	if {[string compare $quartus(project) "LogicalStep_Lab1"]} {
		puts "Project LogicalStep_Lab1 is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists LogicalStep_Lab1]} {
		project_open -revision LogicalStep_Lab1_top LogicalStep_Lab1
	} else {
		project_new -revision LogicalStep_Lab1_top LogicalStep_Lab1
	}
...
# 	set_global_assignment -name QIP_FILE LogicalStep_124/synthesis/LogicalStep.qip
#	set_global_assignment -name VERILOG_FILE LogicalStep_124_top.v
	set_global_assignment -name TCL_SCRIPT_FILE LogicalStep_124.tcl
